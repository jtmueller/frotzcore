...
namespace ZTools
{
    public static class Txd
    {
        ...
        private static System.Collections.Generic.List<TxH.CRefItemT>? strings_base = null;
        private static System.Collections.Generic.List<TxH.CRefItemT>? routines_base = null;
    ...
    private static ulong dict_start = 0;
        ...
        decode.high_address = (ulong)txio.file_size;
            ...
            if ((decode.LowAddress - code_base) >= txio.story_scaler)
            {
                ...
                if (option_dump > 0)
                {
                    txio.TxPrint("\n[Start of data");
                    if (option_labels == 0)
                        txio.TxPrintf(" at {0:X}", (ulong)code_base);
                    txio.TxPrint("]\n\n");
                    DumpData(code_base, decode.LowAddress - 1);
                    txio.TxPrint("\n[End of data");
                    if (option_labels == 0)
                        txio.TxPrintf(" at {0:X}", (ulong)(decode.LowAddress - 1));
                    txio.TxPrint("]\n");
                }
            }
        ...
        internal static void DecodeProgram()
        {
            ...
            else
            {
                txio.TxPrint("\n[Start of code");
                if (option_labels == 0)
                    txio.TxPrintf(" at {0:X}", (ulong)decode.LowAddress);
                txio.TxPrint("]\n");
                ...
                txio.TxPrint("\n[End of code");
                if (option_labels == 0)
                    txio.TxPrintf(" at {0:X}", (ulong)decode.Pc);
                txio.TxPrint("]\n");
            }
            ...
            else
            {
                if (decode.Pc == start_data_pc)
                {
                    if (option_dump > 0)
                    {
                        txio.TxPrint("\n[Start of data");
                        if (option_labels == 0)
                            txio.TxPrintf(" at {0:X}", (ulong)start_data_pc);
                        txio.TxPrint("]\n\n");
                        DumpData(start_data_pc, end_data_pc - 1);
                        txio.TxPrint("\n[End of data");
                        if (option_labels == 0)
                            txio.TxPrintf(" at {0:X}", (ulong)(end_data_pc - 1));
                        txio.TxPrint("]\n");
                    }
                    decode.Pc = end_data_pc;
                }
        ...
        private static int DecodeCode()
        {
            ...
            do
            {
                if (!decode.FirstPass)
                {
                    ...
                    {
                        txio.TxPrintf("{0:X5}:  ", (ulong)decode.Pc);
                    }
                }
